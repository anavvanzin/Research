...
Almost all thesis work lives here. It has its own detailed `CLAUDE.md` — **always read it first when working inside the hub**. Cross-workspace facts:
...
- Every major sub-repo (`hub/iconocracy-corpus`, `apps/iconocracia-companion`, `apps/webiconocracy`, `pipelines/Atlas`) is its own Git repository with its own `CLAUDE.md`, `package.json`, or `environment.yml`. Run commands **inside the specific sub-repo**, not at `~/Research/` root.
