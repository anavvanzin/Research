...
- Every major sub-repo (`hub/iconocracy-corpus`, `apps/iconocracia-companion`, `pipelines/Atlas`) is its own Git repository with its own `CLAUDE.md`, `package.json`, or `environment.yml`. Run commands **inside the specific sub-repo**, not at `~/Research/` root.
